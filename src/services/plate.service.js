...
//   APIBRASIL_BEARER_TOKEN  — Bearer token do painel apibrasil.io
//   APIBRASIL_DEVICE_TOKEN  — Device token do painel apibrasil.io
...
export const getVehicleByPlate = async (plate) => {
  ...
  if (BEARER_TOKEN && DEVICE_TOKEN) {
    try {
      ...
      if (!response.ok || data.error) throw new Error(data.message || "Erro na API Brasil");
  ...
  console.log(`[MOCK] Placa ${normalized} — configure APIBRASIL_BEARER_TOKEN e APIBRASIL_DEVICE_TOKEN para dados reais`);
